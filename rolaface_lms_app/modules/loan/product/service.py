...
def create_loan_product(data: dict):
    ...
    if not data.get("broken_period_interest_recovery_account"):
        product.broken_period_interest_recovery_account = data.get(
...
def update_loan_product(product_id: str, data: dict):
    ...
    if not data.get("broken_period_interest_recovery_account") and data.get(
        "interest_income_account"
    ):
        if product.get("broken_period_interest_recovery_account") != data.get(
            "interest_income_account"
        ):
            product.broken_period_interest_recovery_account = data.get(
                "interest_income_account"
...
def get_loan_products(
    args: dict, page: int, page_size: int, sort_by="creation", sort_order="desc"
):
...
def delete_loan_product(product_id: str):
    ...
    linked_loans = frappe.db.count(
        "Loan", {"loan_product": product_id, "docstatus": ["!=", 2]}
    )
    if linked_loans > 0:
        raise frappe.ValidationError(
            f"Cannot delete Loan Product '{product_id}' because it is linked to {linked_loans} active loan(s). Disable it instead."
        )
...
def toggle_loan_product_status(product_id: str, disable_flag: int):
    ...
    if product.disabled == disable_flag:
        ...
        raise frappe.ValidationError(
            f"Loan Product '{product.product_name}' is already {action}."
        )

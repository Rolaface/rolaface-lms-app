...
def get_repayment_history(loan_id: str, page: int = 1, page_size: int = 20, search: str = None) -> Tuple[list, int, int]:
    try:
        ...
        query = (
            frappe.qb.from_(lr)
            .select(
                lr.name, lr.posting_date, lr.mode_of_payment, lr.amount_paid,
                lr.principal_amount_paid, lr.total_interest_paid, lr.total_penalty_paid,
                lr.total_charges_paid, lr.pending_principal_amount
            )
...
def get_disbursement_history(loan_id: str, page: int = 1, page_size: int = 20, search: str = None) -> Tuple[list, int, int]:
    try:
        if not frappe.db.exists("Loan", loan_id):
            raise frappe.DoesNotExistError(f"Loan '{loan_id}' does not exist.")
        ...
        query = (
            frappe.qb.from_(ld)
            .select(
                ld.name, ld.loan_product, ld.disbursement_date, ld.disbursement_account,
                ld.sanctioned_loan_amount, ld.disbursed_amount,
                ld.current_disbursed_amount.as_("disbursed_till_now"),
                ld.status, ld.tranche_number, ld.reference_number,
                ld.monthly_repayment_amount, ld.repayment_method,
                ld.repayment_start_date, ld.repayment_schedule_type,
                ld.mode_of_payment, ld.bank_account
            )
...
def get_repayment_schedule_versions(loan_id: str) -> List[Dict[str, Any]]:
    try:
        ...
        query = (
            frappe.qb.from_(lrs)
            .select(
                lrs.name.as_("id"), lrs.repayment_start_date.as_("active_from"),
                lrs.maturity_date.as_("active_till"), lrs.creation,
                lrs.status, lrs.creation.as_("valid_from"),
...
def get_repayment_schedule_summary(loan_id: str, schedule_id: str = None) -> Dict[str, Any]:
    try:
        ...
        parent_query = (
            frappe.qb.from_(lrs)
            .select(
                lrs.name, lrs.repayment_start_date, lrs.maturity_date,
                lrs.monthly_repayment_amount, lrs.total_installments_paid,
                lrs.total_installments_raised, lrs.total_installments_overdue,
        ...
        child_query = (
            frappe.qb.from_(rs)
            .select(
                rs.idx, rs.payment_date, rs.number_of_days,
                rs.principal_amount, rs.interest_amount, rs.total_payment,
                rs.balance_loan_amount, rs.charges, rs.demand_generated
            )
            .where(
...
def get_repayment_schedule_timeline(loan_id: str) -> Dict[str, Any]:
    try:
        ...
        raw_schedule = (
            frappe.qb.from_(rs)
            .select(
                rs.idx, rs.payment_date, rs.total_payment,
...
def get_installment_detail(loan_id: str, installment_idx: int) -> Dict[str, Any]:
    try:
        ...
        detailed_row = (
            frappe.qb.from_(rs)
            .select(
                rs.idx, rs.payment_date, rs.principal_amount.as_("principal"),
                rs.interest_amount.as_("interest"), rs.total_payment,
                rs.balance_loan_amount, rs.charges
            )
...
def get_loan_accounting_ledger(loan_id: str, page: int = 1, page_size: int = 20, search: str = None) -> Tuple[list, int, int]:
    try:
        ...
        query = (
            frappe.qb.from_(gl)
            .select(
                gl.posting_date, gl.voucher_type, gl.voucher_no.as_("description"),
                gl.debit, gl.credit, gl.account
            )
...
def get_collateral_view(loan_id: str, page: int = 1, page_size: int = 20, search: str = None) -> Tuple[list, int, int]:
    try:
        assignments = frappe.get_all(
            "Loan Security Assignment",
            filters={"loan": loan_id, "docstatus": ["<", 2]},
            fields=[
                "name", "status", "applicant_type", "applicant",
                "total_security_value", "maximum_loan_value",
        ...
        for assignment_info in assignments:
            assignment_doc = frappe.get_doc("Loan Security Assignment", assignment_info.name)
            items = []
...
def get_loan_documents(loan_id: str, page: int = 1, page_size: int = 20, search: str = None) -> Tuple[list, int, int]:
    try:
        f = DocType("File")
        query = (
            frappe.qb.from_(f)
            .select(
                f.name, f.file_name, f.file_url, f.file_size,
                f.creation, f.is_private
            )
        ...
        if search:
            ...
            records = [
                r for r in records
                if search_term in str(r.get("file_name", "")).lower()
            ]
...
def get_loan_activity_audit(loan_id: str, page: int = 1, page_size: int = 20, search: str = None) -> Tuple[list, int, int]:
    try:
        if not frappe.db.exists("Loan", loan_id):
            raise frappe.DoesNotExistError(f"Loan '{loan_id}' does not exist.")
        ...
        for c in comments:
            records.append({
                "id": c["id"], "type": "note", "timestamp": c["timestamp"], 
                "actor": c["actor"], "title": "Account Note", 
        ...
        ).where((comm.reference_doctype == "Loan") & (comm.reference_name == loan_id)).run(as_dict=True)
        ...
        for c in comms:
            ...
            records.append({
                "id": c["id"], "type": c_type, "timestamp": c["timestamp"], 
                "actor": c["actor"], "title": c["subject"] or f"Outbound {c_type}", 
                "subtitle": frappe.utils.strip_html_tags(c["content"])[:150]
            })
        ...
        ).where((lr.against_loan == loan_id) & (lr.docstatus == 1)).run(as_dict=True)

        for r in repayments:
            records.append({
                "id": r["id"], "type": "system", "timestamp": r["timestamp"], 
                "actor": r["actor"], "title": "Repayment processed", 
        ...
        accruals = frappe.qb.from_(lia).select(
            ...
            lia.interest_amount
        ).where((lia.loan == loan_id) & (lia.docstatus == 1)).run(as_dict=True)

        for a in accruals:
            if a.get("interest_amount"):
                records.append({
                    "id": f"{a['id']}_int", "type": "system", "timestamp": a["timestamp"], 
                    "actor": "System", "title": "Interest accrued for the current cycle", 
        ...
        ).where((lrs.loan == loan_id) & (lrs.docstatus == 1)).run(as_dict=True)

        for s in schedules:
            is_restructure = bool(s["amended_from"])
            records.append({
                "id": s["id"], "type": "system", "timestamp": s["timestamp"], 
                "actor": s["actor"], "title": "Loan restructured" if is_restructure else "Initial Repayment Schedule Generated", 
        ...
        ).where((version.ref_doctype == "Loan") & (version.docname == loan_id)).run(as_dict=True)
        ...
        for v in versions:
            ...
            if changed_fields and not all(f in ["modified", "docstatus"] for f in changed_fields):
                records.append({
                    "id": v["id"], "type": "system", "timestamp": v["timestamp"], 
                    "actor": v["actor"], "title": "Account reviewed / updated", 
